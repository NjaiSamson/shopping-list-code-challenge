...
// Maintaining track of local storage memory
  let itemsShoppingList = JSON.parse(localStorage.getItem('itemsShoppingList')) || [];
...
// Function to render list of items in the DOM  
  function renderItemsList() {
      ...
      itemsShoppingList.forEach((item, items) => { 
          ...
          markPurchasedButton.textContent = item.purchased ? 'Item Purchased' : 'Mark Purchased';
          markPurchasedButton.className = item.purchased ? 'purchased-button' : 'mark-button';
...
// Function to check if an item exist in the list and to items to the list
  function addItemToList() {
      ...
      if (itemName) { 
          const itemExistInList = itemsShoppingList.findIndex(item => item.name === itemName); 
          if (itemExistInList > -1) { 
              const confirmAddedItem = confirm(`"${itemName}" is already in the list. Do you want to add it again?`);               if (!confirmAddedItem) {  
                  addNewItem.value = '';  
                  return;
              }
          }
  ...
  function itemsCheckout(){
      ...
      itemsShoppingList.filter((item, itemRemove) => {
          const removeItem = confirm(`Do you want to remove ("${item.name}") from your shopping list?`);
          if (removeItem) {
      ...
      itemsShoppingList = itemsShoppingList.map(item => ({ ...item, purchased: true }));
      updateLocalStorage();
      renderItemsList();
  ...
  const updateLocalStorage = () => {
      localStorage.setItem('itemsShoppingList', JSON.stringify(itemsShoppingList));
  };
